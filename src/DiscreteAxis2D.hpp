...
public:
  ...
  DiscreteAxis2D(const double Min_First, const double Max_First,
                 ...
                 const double Max_Second, const double Step_Second,
  ...
  DiscreteAxis2D(const DiscreteAxis2D& rhs, const double Initial_Value):
    Min_First_(rhs.Min_First_),
    Max_First_(rhs.Max_First_),
    Step_First_(rhs.Step_First_),
    Center_Min_First_(rhs.Center_Min_First_),
    Inv_Step_First_(rhs.Inv_Step_First_),
    Min_Second_(rhs.Min_Second_),
    Max_Second_(rhs.Max_Second_),
    Step_Second_(rhs.Step_Second_),
    Center_Min_Second_(rhs.Center_Min_Second_),
    Inv_Step_Second_(rhs.Inv_Step_Second_),
    Num_Bins_First_(rhs.Num_Bins_First_),
    Num_Bins_Second_(rhs.Num_Bins_Second_),
    Array_(rhs.Array_.size(), Initial_Value)
  {};
  ...
  double get_entry(const int index) const noexcept{
    return Array_[index];
  }

  /// increment operator on histogram element corresponding to value_first and
  /// value_second
  void add_one(const std::pair<double, double> &value) noexcept {
  ...
  double get_min_first() const noexcept { return Min_First_; }
  /// return center of first bin of first dimension
  double get_center_min_first() const noexcept { return Center_Min_First_; }
  /// return minimal range of second dimension
  double get_min_second() const noexcept { return Min_Second_; }
  /// return center of first bin of second dimension
  double get_center_min_second() const noexcept { return Center_Min_Second_; }
  /// return maximal range of first dimension
  double get_max_first() const noexcept { return Max_First_; }
  /// return center of last bin of first dimension
  double get_center_max_first() const noexcept {
    return Max_First_ - 0.5 * Step_First_;
  }
  /// return maximal range of second dimension
  double get_max_second() const noexcept { return Max_Second_; }
  /// return center of last bin of second dimension
  double get_center_max_second() const noexcept {
    return Max_Second_ - 0.5 * Step_Second_;
  }
  /// get step range of first dimension
  double get_step_first() const noexcept { return Step_First_; }
  /// get step range of second dimension
  double get_step_second() const noexcept { return Step_Second_; }
  /// get number of bins in first dimension
  int get_num_bins_first() const noexcept { return Num_Bins_First_; }
  /// get number of bins in second dimension
  int get_num_bins_second() const noexcept { return Num_Bins_Second_; }
  /// get corresponding values of bin with index bin_
  std::pair<double, double> get_value(const int bin) const noexcept{
  ...
  double get_value_first(const int bin_first) const noexcept {
    return Center_Min_First_ + Step_First_ * bin_first;
  }
  /// get corresponding value from second dimension of bin  with index bin_
  double get_value_second(const int bin_second) const noexcept {
    return Center_Min_Second_ + Step_Second_ * bin_second;
  }

  Range get_range_first() const noexcept{
    return Range{Min_First_, Step_First_, Max_First_};
  }

  Range get_range_second() const noexcept{
    return Range{Min_Second_, Step_Second_, Max_Second_};
  }

  bool is_in_boundaries(const double value_first, const double value_second) const noexcept{
...
inline int DiscreteAxis2D::get_bin(const double value_first,
                                   const double value_second) const noexcept {
