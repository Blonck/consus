...
int main(int argc, char const *argv[])
{
  ...
  results = reweight_dT<NVT>(DOS, MicroMeans[column], Betas);
  ...
  for (size_t i = 0; i < num_bins; ++i) {
    ...
    for (size_t j = 0; j < Betas.size(); ++j) {
      results_jk[j][i] =
          reweight_dT<NVT>(jk_DOS, jk_MicroMeans[column], Betas[j]);
    }
  }
  ...
  std::string filename = pathr + "/" + replacediv(header[column]) + "_dT.dat";
