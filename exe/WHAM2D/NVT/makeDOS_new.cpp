...
int main(int argc, char const *argv[])
{
  ...
  for (size_t i = 0; i < filenames.size(); ++i) {
    ...
    if (i == 0) {
      ...
      lnZ.push_back(lnZ[i-1] + estimate_lnZ_from_hist<NVT>(
                                 Hist, HistInfo, Parameters[i-1], Parameters[i]));
    }
  }

  std::cout << "last full WHAM" << "\n";
  calc_logDOS_full_broydn2<NVT>(Hist, HistInfo, HistInfos, Parameters, devmax,
                            lnZ, logDOS);
